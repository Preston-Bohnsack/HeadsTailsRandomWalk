...
public class Main{
  ...
  public static void simWalk(int numwalks, int steps, double upHeight, double downHeight, double startingHeight){
    double[][] walks = manyWalks(numwalks, steps, upHeight, downHeight, startingHeight);
    walks = floor(walks); // makes sure each walk cannot continue if their points are <= 0
    System.out.println(numBustWalks(walks) + " out of " + walks.length + " walks were a bust");
    double avgROI = 0;
  ...
  public static double[][] manyWalks(int numWalks, int steps, double upHeight, double downHeight, double startingHeight){
  ...
  public static double[] walk(int steps, double upHeight, double downHeight, double startingHeight){
    double[] walk = new double[steps + 1];
    walk[0] = startingHeight;
    for(int step = 1; step <= steps; step++){
      walk[step] = walk[step - 1] +
  	((Math.random() < 0.5) ? -downHeight : upHeight);
    }
    // printWalk(walk);
